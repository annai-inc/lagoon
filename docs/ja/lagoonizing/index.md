# 既存のサイトをLagoonizingする

「Lagoonizing」とは、既存のサイトをLagoonプラットフォームに適応させることで、通常は難しくない作業です（サイトやセットアップにより異なります）。既存サイトをLagoonで動かすためのセットアップにはいくつかの手順が必要となります。このプロセスを簡単にするためのステップバイステップのガイドをまとめています。
